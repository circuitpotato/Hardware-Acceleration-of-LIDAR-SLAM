...
#include <unistd.h>
#include <esp_log.h>
#include <nvs_flash.h>/* Simple HTTP Server Example
...
static esp_err_t pose_get_handler(httpd_req_t *req)
{
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ...
            if (httpd_query_key_value(buf, "x_value", param, sizeof(param)) == ESP_OK) {
                ESP_LOGI(TAG, "transmitting serial data to the terminal");
                char serial_output_for_pose[6];
                sprintf(serial_output_for_pose, "%s","0002!");
                // http_req_get_hdr doesn't work because it's for headers
                ESP_LOGI(TAG, "Transmitted Data: %s",serial_output_for_pose);
                tinyusb_cdcacm_write_queue(0, &serial_output_for_pose, 6);
                tinyusb_cdcacm_write_flush(0, 0);

            }
            if (httpd_query_key_value(buf, "y_value", param, sizeof(param)) == ESP_OK) {
                ESP_LOGI(TAG, "transmitting serial data to the terminal");
                char serial_output_for_pose[6];
                sprintf(serial_output_for_pose, "%s","0004!");
                // http_req_get_hdr doesn't work because it's for headers
                ESP_LOGI(TAG, "Transmitted Data: %s",serial_output_for_pose);
                tinyusb_cdcacm_write_queue(0, &serial_output_for_pose, 6);
                tinyusb_cdcacm_write_flush(0, 0);
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ...
            if (httpd_query_key_value(buf, "lidar_scan", param, sizeof(param)) == ESP_OK) {
                ...
                ESP_LOGI(TAG, "transmitting serial data to the terminal");
                static char serial_output_for_csv_read[5];
                serial_output_for_csv_read[0] = 0x2b;
                // http_req_get_hdr doesn't work because it's for headers
                for(int i =0; i <4;i++) serial_output_for_csv_read[i + 1] = scan_count[i];
                serial_output_for_csv_read[5] = 0x2b;
                ESP_LOGI(TAG, "Transmitted Data: %s",serial_output_for_csv_read);
                tinyusb_cdcacm_write_queue(0, &serial_output_for_csv_read, 6);
                tinyusb_cdcacm_write_flush(0, 0);
             ...
             if (httpd_query_key_value(buf, "lidar_increment", param, sizeof(param)) == ESP_OK) {
                ...
                memcpy(scan_count,param,4);
                // http_req_get_hdr doesn't work because it's for headers
                for(int i =0; i <4;i++) serial_output_for_csv_read[i + 1] = scan_count[i];
                serial_output_for_csv_read[5] = 0x2a;
                ESP_LOGI(TAG, "Transmitted Data: %s",serial_output_for_csv_read);
                tinyusb_cdcacm_write_queue(0, &serial_output_for_csv_read, 6);
                tinyusb_cdcacm_write_flush(0, 0);
            ...
            if (httpd_query_key_value(buf, "lidar_increment_size", param, sizeof(param)) == ESP_OK) {
                ...
                // http_req_get_hdr doesn't work because it's for headers
                for(int i =0; i <4;i++) serial_output_for_csv_read[i + 1] = scan_count[i];
                serial_output_for_csv_read[5] = 0x7c;
                ESP_LOGI(TAG, "Transmitted Data: %s",serial_output_for_csv_read);
                tinyusb_cdcacm_write_queue(0, &serial_output_for_csv_read, 6);
                tinyusb_cdcacm_write_flush(0, 0);
...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(server, &pose);
        #if CONFIG_EXAMPLE_BASIC_AUTH
