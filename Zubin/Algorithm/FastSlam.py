...
def processSensorData(pf, sensorData, plotTrajectory = True):
    ...
    for key in sorted(sensorData.keys()):
        ...
        plt.savefig(r"C:\Games\SLAM-2D-LIDAR-SCAN\Output" + "\%s"%str(count).zfill(3) + '.png')
        plt.close()
...
def main():
    ...
    sensorData = readJson(r"C:\Games\SLAM-2D-LIDAR-SCAN\DataSet\PreprocessedData\intel_gfs")
