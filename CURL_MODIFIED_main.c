...
void readDatasetLineByLine(CURL* curl ,CURLcode res ,int line_number){
    ...
    char url_query[50];
    sprintf(url_query,"http://localhost:8080?param=%d",line_number);
...
int main(){
    ...
    readDatasetLineByLine(curl, res,0);  // read 1st line of code (scan 0)
    ...
    for (scan_iter = 1; scan_iter < 100; scan_iter++){
        ...
        readDatasetLineByLine(curl,res,scan_iter);  // read current line of code starting from scan 1
        readAScan(lidar.range_min, lidar.angles, lidar.range_max, 24);
